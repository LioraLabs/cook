...
export interface CookHighlightOptions {
  wasmPath: string;     // absolute path to tree-sitter-cook.wasm
  queryPath: string;    // absolute path to highlights.scm
}

// Capture-name → CSS class. The highlights query in tree-sitter-cook uses
// standard capture names; we map them to hl-* classes used by spec.css.
...
function renderHighlighted(source: string, parser: TsParser, query: TsQuery): string {
  ...
  // Build a sorted list of non-overlapping ranges. Tree-sitter captures may
  // overlap when a node has multiple captures; we pick the longest/outermost.
  ...
  // Stitch: walk source, emitting spans at range boundaries.
  let out = '';
  let cursor = 0;
...
export function remarkCookHighlight(options: CookHighlightOptions) {
  return async function transformer(tree: Root) {
    ...
    for (const node of pending) {
      ...
      // Replace the Code node with an HTML node that emits a <pre><code>
      // with the pre-highlighted content.
