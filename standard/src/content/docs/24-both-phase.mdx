...
`cook.dep_output` and `cook.dep_output_list` are the Lua-side counterpart of the `$<NAME>` string-substitution surface (§{xref.string-substitution}). A `>{ … }`, `plate >{ … }`, or `test >{ … }` body that needs to reference another recipe's output uses these functions instead of textual placeholder substitution.
