...
```ebnf
gather_step ::= "gather" input+ NEWLINE
input       ::= STRING | "!" STRING
```

This defines `gather_step` as the keyword `gather` followed by one or more inputs and a newline. Each input is either a bare string (include) or `!` followed by a string (exclude).
