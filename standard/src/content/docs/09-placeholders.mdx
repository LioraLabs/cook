...
| `$<NAME>` (NAME is a declared chore parameter, §7.1.1) | chore bodies only | The bound parameter value (string), or the space-separated, individually-shell-quoted joined form for variadic parameters. Scope-limited to chore bodies that declare the parameter. Resolved at register phase, before the shell step is dispatched. |
