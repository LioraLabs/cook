...
The `MemberSource source=ProbeKey("scenes")` desugar node is identical to the node produced by `ingredients scenes` in `positive/ingredients-probe` — confirming that `$<recipe[in]>` placeholders do not alter the parse-level AST; the accessor is resolved entirely at the codegen layer.
