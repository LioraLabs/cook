...
which (`test = {...}` and `seal = {...}`) collide lexically with real
step keywords (`test_step`'s `test` and the recipe-level `seal`
...
`seal = {...}` continuation line is ever mistaken for a `test_step` or
a `seal` opener. This is the one-step invariant the cook_cc 0.13.0
rewrite depends on: the whole call parses to a single `Step::InlineLua`, not
a `test_step`/`seal` misparse followed by parse garbage.
