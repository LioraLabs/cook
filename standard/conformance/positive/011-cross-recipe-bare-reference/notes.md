Pins the § 5.5 bare-name cross-recipe reference. `app`'s shell body contains `{libmath}`, which per the Standard expands at register time to the space-joined output list of `libmath`.
