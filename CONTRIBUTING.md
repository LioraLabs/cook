...
The Cookfile language is defined by the Cook Standard in [`standard/`](standard/). The Standard is the authoritative reference for the language; the Rust parser in `cli/crates/cook-lang/` is the reference implementation.
...
2. Add one entry to `standard/src/content/docs/appendix/E-changes.mdx` with a new stable `CS-NNNN` ID, following the house shape: date, version, sections affected, summary, backward compatibility, and conformance notes.
...
The repo ships a portable `pre-commit` hook at `.githooks/pre-commit` that inspects the staged diff and rejects the commit when you've touched language-surface code without also touching `standard/`. Install it once per clone:
...
- `tree-sitter-cook/scripts/conformance.mjs` walks the same corpus and asserts every positive parses cleanly and every syntactic negative produces an `ERROR`/`MISSING` node. Negatives whose rule is semantic rather than syntactic (register-phase and codegen checks with no parse-level footprint) are recorded as accepted, each with a note naming the rule. Run it with `node scripts/conformance.mjs` from `tree-sitter-cook/`, or `npm run conformance`. See CS-0014.

### `cook-lang` conformance workflow

The Rust parser claims a Cook Standard version via the `pub const COOK_STANDARD_VERSION: &str = "X.Y";` constant in `cli/crates/cook-lang/src/lib.rs`. This constant is the single source of truth; the crate README and `cook --version` output mirror it.
...
- **`tree-sitter-cook`** — set the claimed version in a header comment in `grammar.js`, mirrored in `package.json` and `tree-sitter.json`'s `version`/`description` fields. Currently claims v0.14.
