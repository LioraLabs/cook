...
use super::{ident_end, is_ident_cont, is_ident_start, skip_non_code, Skip};
...
pub fn scan_var_reads(source: &str) -> BTreeSet<String> {
    ...
    while i < bytes.len() {
        ...
        // Strings and comments are not code (see the walk in `super`).
        match skip_non_code(source, i) {
            Skip::Ended(next) => {
                i = next;
                continue;
            }
            Skip::Unterminated => return keys,
            Skip::Code => {}
        }
...
fn is_part_of_larger_identifier(bytes: &[u8], i: usize, prefix_len: usize) -> bool {
    ...
    after < bytes.len() && is_ident_cont(bytes[after]) && bytes[after] != b'.'
}
