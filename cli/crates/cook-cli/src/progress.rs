...
use cook_progress::{
    Driver, InlineRenderer as NewInlineRenderer, JsonWriter, LogConfig, LogStore,
    PlainRenderer as NewPlainRenderer,
};
...
/// Spawn the new cook-progress Driver reading from `rx`. Returns a JoinHandle that
...
pub fn spawn_new_renderer(
    ...
    std::thread::spawn(move || {
        ...
        let renderer: Box<dyn cook_progress::NewRenderer> = match mode {
            ...
            OutputMode::Auto => Box::new(NewInlineRenderer::new(ProgressDrawTarget::stderr())),
        };
