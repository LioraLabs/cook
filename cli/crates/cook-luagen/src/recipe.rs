...
pub fn generate_with_names(
    ...
    if cookfile.config_blocks.is_empty() {
        ...
        for (i, use_stmt) in cookfile.uses.iter().enumerate() {
            ...
            let mut line = cook_contracts::module_binding::binding(&use_stmt.module_name);
            if i == 0 {
        ...
        let mut fn_line = format!(
            "function {}(selected_name)",
            cook_contracts::registration::CONFIG_DISPATCH_NAME
        );
        if cookfile.uses.is_empty() {
        ...
        out.push_str(&format!(
            "end function {}()\n",
            cook_contracts::registration::MAIN_PROGRAM_NAME
        ));
    }
        ...
        .map(|p| {
            ...
            let value_expr = match p {
                ...
                | cook_lang::ast::ChoreParam::VariadicStar { .. } => {
                    ...
                    format!("table.concat({}.{n}, \" \")", crate::COOK_PARAMS_LOCAL)
                }
                _ => format!("{}.{n}", crate::COOK_PARAMS_LOCAL),
            };
...
///
/// Serializes from the SHARED `MemberSourceDescriptor` shape with the
/// shared key constants (COOK-390) — the same declaration
...
fn member_source_meta_field(recipe: &Recipe) -> Option<String> {
    ...
    let descriptor = match &step.source {
