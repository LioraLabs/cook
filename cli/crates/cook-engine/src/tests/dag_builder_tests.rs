...
// ---------------------------------------------------------------------------
// COOK-402: the recorded DagStructure IS the wiring — provenance per edge
// ---------------------------------------------------------------------------

/// Shorthand: a sequential shell unit with no cache metadata.
...
/// Resolve a `(recipe, unit_idx)` origin to its dag id in the structure.
fn unit_id(s: &DagStructure, recipe: &str, unit_idx: usize) -> usize {
    s.origins
        .iter()
        .position(|o| {
            matches!(o, NodeOrigin::Unit { recipe: r, unit_idx: i }
                     if r == recipe && *i == unit_idx)
        })
        .unwrap_or_else(|| panic!("no origin for unit:{recipe}:{unit_idx} in {:?}", s.origins))
}

fn has_edge(s: &DagStructure, from: usize, to: usize, kind: EdgeProvenance) -> bool {
    s.edges
        .iter()
        .any(|e| e.from == from && e.to == to && e.kind == kind)
}

/// Every wiring rule leaves its kind on the recorded edge: serial barriers,
/// group entry, probe consumption, coarse barriers, fine dep_order refs.
#[test]
fn structure_records_provenance_per_edge_kind() {
    let producer = recipe("producer", &[], vec![seq_unit("make gen")]);
    let mut consumer = RecipeUnits {
        recipe_name: "consumer".into(),
        deps: vec!["producer".into()],
        units: vec![
            CapturedUnit {
                payload: probe("cc:flags"),
                cache_meta: None,
                dep_kind: DepKind::Sequential,
                probes: vec![],
                unit_env_vars: Default::default(),
    ...
    let (_dag, s) = build_dag_traced(vec![producer, consumer]).expect("builds");

    let p0 = unit_id(&s, "producer", 0);
    let probe0 = unit_id(&s, "consumer", 0);
    let cc_a = unit_id(&s, "consumer", 1);
    let cc_b = unit_id(&s, "consumer", 2);
    let link = unit_id(&s, "consumer", 3);

    // Coarse dep-list barrier: producer leaf -> every consumer root.
    assert!(has_edge(&s, p0, cc_a, EdgeProvenance::Barrier), "{:?}", s.edges);
    assert!(has_edge(&s, p0, cc_b, EdgeProvenance::Barrier), "{:?}", s.edges);
    // Probe consumption.
    assert!(has_edge(&s, probe0, cc_a, EdgeProvenance::Probe), "{:?}", s.edges);
    // Group members become the barrier; the link unit enters sequentially.
    assert!(has_edge(&s, cc_a, link, EdgeProvenance::Serial), "{:?}", s.edges);
    assert!(has_edge(&s, cc_b, link, EdgeProvenance::Serial), "{:?}", s.edges);
    // Fine-grained ref on the link unit, additive with the barrier above.
    assert!(has_edge(&s, p0, link, EdgeProvenance::DepOrder), "{:?}", s.edges);
}

/// The additive rule (CS-0161, "Rejected alternative"), read off the
/// structure: a declared `requires` keeps its whole-recipe barrier whether or
/// not a unit also fine-covers the same producer with `dep_order`.
#[test]
fn structure_keeps_declared_barrier_alongside_fine_cover() {
    let producer = recipe("producer", &[], vec![seq_unit("make gen")]);
    ...
    let p0 = unit_id(&s, "producer", 0);
    let root = unit_id(&s, "consumer", 0);
    let fine = unit_id(&s, "consumer", 1);

    assert!(
        has_edge(&s, p0, root, EdgeProvenance::Barrier),
        "the declared requires barrier must survive fine coverage: {:?}",
        s.edges
    );
    assert!(has_edge(&s, p0, fine, EdgeProvenance::DepOrder), "{:?}", s.edges);
}

/// Leaf pass-through is visible in the structure: a unit-less meta-target
/// forwards its deps' leaves, so a downstream barrier or fine ref lands on
/// the real producer's terminals instead of vanishing.
#[test]
fn structure_routes_edges_through_a_unit_less_meta_target() {
    let producer = recipe("producer", &[], vec![seq_unit("make gen")]);
    ...
    let (_dag, s) = build_dag_traced(vec![producer, middle, consumer]).expect("builds");

    let p0 = unit_id(&s, "producer", 0);
    let c0 = unit_id(&s, "consumer", 0);

    assert!(
        has_edge(&s, p0, c0, EdgeProvenance::Barrier),
        "the coarse dep on a unit-less middle must forward to the producer's \
         leaves: {:?}",
        s.edges
    );
    assert!(
        has_edge(&s, p0, c0, EdgeProvenance::DepOrder),
