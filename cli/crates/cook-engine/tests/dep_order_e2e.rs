...
/// The flat-wave half: with the requires edge fine-covered, the consumer's
/// NON-consuming unit must start without waiting for the producer's last
/// unit. The producer sleeps ~1.5s before writing; the consumer's first unit
/// is instant. Under the old root-barrier lowering, `first.txt` could only
/// be written AFTER `build/gen.a`; with suppression it lands well before.
/// mtime comparison with a 1.5s producer delay leaves a wide margin against
/// scheduler jitter on a loaded test box.
#[test]
...
"#,
    ...
    assert!(
        first < gen,
        "the non-consuming unit must not wait for the producer's last unit \
         (fine-covered requires edge ⇒ no coarse root barrier); first.txt \
         was written at {first:?}, gen.a at {gen:?}:\n{combined}"
    );
}
