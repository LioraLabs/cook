...
`stage_luarocks` (inside `dist.package`) copies `src/luarocks/` (the
pure-Lua library tree) into `target/cook-stage/share/luarocks/`. The upstream `Makefile`,
...
7. `cook package` runs the `verify` helper as its final step on Linux
   x86_64 and darwin-arm64 — the bundled luarocks must still install
   lua-cjson against the bundled lib + headers.
