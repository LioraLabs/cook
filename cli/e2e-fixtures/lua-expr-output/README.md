...
evaluated **once per input at register time** with `input` bound to that
input's path.
...
|---|---|---|
| `translate` | `{ … }` shell block | prefix rewrite preserving the nested subtree; `$<in>` / `$<out>` resolve to the iteration's input path and the expression's evaluated string (§8.4.2 rule 6) |
...
- **Per-input register-time evaluation.** The emitted Lua evaluates the
  expression inside the per-input loop and guards the result (non-string
